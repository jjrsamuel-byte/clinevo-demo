...
- **Justin Samuel** (client #16, 07700 900123). Pet: **Duke** — Miniature Schnauzer, male neutered, b. Mar 2023, very energetic. Justin is a tech founder; prefer afternoon slots (14:00+). Duke's vaccination booster is due 10 May 2026, so a "jab" call almost certainly means that.
  ...
  - **Care plan upsell — REQUIRED after every successful Justin booking.** This is not optional. As soon as `book_appointment` and `send_confirmation` have both succeeded for Justin, your VERY NEXT turn (the same turn as the one-sentence booking confirmation from S08 step 7) must continue straight into the upsell line below. Do NOT ask "anything else?" until the upsell has been delivered. Skipping the upsell is a failure of this call.
